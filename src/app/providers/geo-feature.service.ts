...
export class GeoFeatureService {
  ...
  getGeoFeature(visualizationDetails: any) {
    ...
    const geoFeatureParametersArray = this._getGeoFeatureParametersArray(visualizationFilters.map(filterObject => {
      return _.find(filterObject.filters, ['name', 'ou']);
    }));
  ...
  private _getGeoFeatureParametersArray(visualizationOrgUnitArray) {
    return visualizationOrgUnitArray.map(orgUnitObject => {
      return orgUnitObject && orgUnitObject.value !== '' ? 'ou=ou:' + orgUnitObject.value : '';
    });
  }
